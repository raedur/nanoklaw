...
│   ├── k8s-runner.ts              # Spawns agent K8s Jobs with PVC mounts (primary runner)
│   ├── k8s-runtime.ts             # K8s cluster connectivity check and orphan Job cleanup
│   └── container-runner.ts        # Spawns Docker/Apple Container agents (legacy)
│
├── container/
...
│
├── Dockerfile                     # Orchestrator image (K8s deployment)
│
├── k8s/                           # Kubernetes manifests
│   ├── pvc.yaml                   # PersistentVolumeClaim (ReadWriteMany, shared by orchestrator + agents)
│   ├── rbac.yaml                  # ServiceAccount + Role + RoleBinding for Job management
│   ├── secret-template.yaml       # Template for nanoclaw-secrets (API keys)
│   └── orchestrator-deployment.yaml # Orchestrator Deployment spec
│
...
// Paths — env vars allow K8s override (PVC mounted at /data)
...
// Container / K8s agent configuration
export const K8S_AGENT_IMAGE = process.env.K8S_AGENT_IMAGE || 'nanoclaw-agent:latest';
...
export const IDLE_TIMEOUT = parseInt(process.env.IDLE_TIMEOUT || '1800000', 10); // 30min — keep alive after last result
export const MAX_CONCURRENT_CONTAINERS = Math.max(1, parseInt(process.env.MAX_CONCURRENT_CONTAINERS || '5', 10) || 5);

export const TRIGGER_PATTERN = new RegExp(`^@${ASSISTANT_NAME}\\b`, 'i');
```

**Note:** Paths must be absolute for container volume mounts to work correctly. In K8s, set `DATA_DIR=/data GROUPS_DIR=/data/groups STORE_DIR=/data/store` via the Deployment env vars.
